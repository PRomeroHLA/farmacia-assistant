...
_EXTRACT_PROMPT = """Analiza el siguiente texto de un caso clínico y extrae la información estructurada.
...
- "symptoms": lista de strings con los nombres/labels de los síntomas detectados (ej. ["Dolor", "Fiebre"]). Lista vacía [] si no hay ninguno
- "hypotheses": lista de strings con las hipótesis diagnósticas sugeridas. Deducelas de los simtomas si es posible. (ej. ["Faringitis leve"]). Lista vacía [] si no hay ninguna
...
class OpenAICaseStructureExtractor(CaseStructureExtractor):
    ...
    def extract(self, text: str) -> StructuredCase:
        ...
        prompt = _EXTRACT_PROMPT.format(case_text=text.strip() or "(sin texto)")
